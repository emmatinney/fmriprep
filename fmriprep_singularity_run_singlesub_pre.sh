...
LOCAL_FREESURFER_DIR="${BIDS_DIR}/derivatives/freesurfer_7.2.0/"
...
SINGULARITY_CMD="singularity run --cleanenv -B /shared:/shared -B $BIDS_DIR:/data -B ${TEMPLATEFLOW_HOST_HOME}:${SINGULARITYENV_TEMPLATEFLOW_HOME} -B /scratch/$USER/fmriprep_work:/work -B ${LOCAL_FREESURFER_DIR}:/fsdir $CONTAINER_PATH/$CONTAINER_NAME"
...
cmd="${SINGULARITY_CMD} /data /data/${DERIVS_DIR} participant --participant-label 03 -w /work/ -vv --omp-nthreads 8 --nthreads 12 --mem_mb 30000 --output-spaces MNI152NLin2009cAsym:res-2 anat fsnative fsaverage5 --use-aroma --fs-subjects-dir /fsdir --skip_bids_validation --bids-filter-file /data/filter-file.json"
